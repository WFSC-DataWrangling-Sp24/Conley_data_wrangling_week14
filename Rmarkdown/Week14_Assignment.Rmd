...
(b) Use `mclapply` to do the same thing. Remember to set up the number of cores you will use, first.
    ...
    head(seq_list)
    #2a
    square_root <- lapply(seq_list, sqrt)
    square_root <- do.call(rbind, square_root)
    head(square_root)

    #2b
    nCores <- detectCores()/2
    square_root <- mclapply(seq_list, sqrt, mc.cores = nCores)
    square_root <- do.call(rbind, square_root)
    head(square_root)
    ```
...
(d) Go back through all of your code for question 3 and explicitly call all of the functions that you use. The ones you've written yourself do not need to be explicitly called.
    ...
    #3b
    masses <- foreach(i = 1:nrow(dinosaur), .combine = c) %do% {
      mass_from_length(dinosaur$length[i], dinosaur$species[i])
    }

    #3c
    doParallel::registerDoParallel(nCores)

    masses <- foreach(i = 1:nrow(dinosaur), .combine = c) %dopar% {
      mass_from_length(dinosaur$length[i], dinosaur$species[i])
    }
...
(c) Do the same as in (b) but use parallel processing and the `%dopar%` operator.
    ...
    ```{r}
    #4a
    change_in_forest <- numeric()
    for (i in 1:nrow(forest)) {
      change <- forest[i, "2021"] - forest[i, "1990"]
      change_in_forest <- c(change_in_forest, change)
    }
    head(change_in_forest)

    #4b
    change_in_forest_list <- foreach(i = 1:nrow(forest), .combine = c) %do% {
      change <- forest[i, "2021"] - forest[i, "1990"]
      data.frame(country = rownames(forest)[i], change = change)
    }

    #4c
    doParallel::registerDoParallel(nCores)

    change_in_forest_list <- foreach(i = 1:nrow(forest), .combine = rbind) %dopar% {
      change <- forest[i, "X2021"] - forest[i, "X1990"]
      data.frame(country = rownames(forest)[i], change = change)
    }
